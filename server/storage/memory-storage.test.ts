...
test("public and DM-only characters are isolated and receive spirit-die pools", async () => {
  const storage = new MemStorage(null);
  const publicCharacter = await storage.createCharacterWithSpiritDice({
    name: "Public",
    path: "Path",
    level: 3,
  });
  const firstDmCharacter = await storage.createDmCharacterWithSpiritDice(
    "dm-1",
    { name: "First DM", path: "Path", level: 6 },
  );
  await storage.createDmCharacterWithSpiritDice("dm-2", {
    name: "Second DM",
    path: "Path",
    level: 8,
  });
  ...
  assert.deepEqual(
    (await storage.getDmCharacters("dm-1")).map((character) => character.id),
    [firstDmCharacter.id],
  );
  assert.equal(firstDmCharacter.isDmOnly, true);
  assert.equal(firstDmCharacter.dmOwnerId, "dm-1");
  assert.deepEqual(
    (await storage.getSpiritDiePool(firstDmCharacter.id))?.currentDice,
    ["d4", "d6"],
  );
});
...
test("deleting a character cascades every character-owned resource", async () => {
  const storage = new MemStorage(null);
  const character = await storage.createCharacterWithSpiritDice({
    name: "Character",
    path: "Path",
  ...
  assert.equal(await storage.deleteCharacter(character.id), false);
  assert.equal(await storage.getCharacter(character.id), undefined);
  assert.equal(await storage.getSpiritDiePool(character.id), undefined);
  assert.deepEqual(await storage.getTechniques(character.id), []);
  assert.deepEqual(await storage.getActiveEffects(character.id), []);
  assert.deepEqual(await storage.getGlossaryTerms(character.id), []);
  assert.deepEqual(await storage.getTrackers(character.id), []);
  assert.deepEqual(await storage.getTechniquePreferences("user-1"), []);
});

test("card game state writes reject stale clients and return defensive copies", async () => {
  const storage = new MemStorage(null);
  assert.equal(await storage.getCardGameState(), undefined);

  const first = await storage.upsertCardGameState({ cards: [] }, 0);
  assert.equal(first.conflict, false);
  assert.equal(first.state.updatedAt, first.updatedAt);

  const stale = await storage.upsertCardGameState(
    { cards: [{ id: "stale" }] },
    first.updatedAt - 1,
  );
  assert.equal(stale.conflict, true);
  assert.deepEqual(stale.state.cards, []);

  const futureDated = await storage.upsertCardGameState(
    { cards: [{ id: "future" }] },
    first.updatedAt + 60_000,
  );
  assert.equal(futureDated.conflict, true);
  assert.deepEqual(futureDated.state.cards, []);

  const second = await storage.upsertCardGameState(
    { cards: [{ id: "new" }] },
    first.updatedAt,
  );
  ...
  (second.state.cards as Array<{ id: string }>)[0].id = "mutated";
