...
import type { Character, DmGlossaryTerm, DmScratchpad, DmStack } from "@shared/schema";
...
export function useDmCharacters(userId: string) {
  return useQuery<Character[]>({
    queryKey: dmQueryKeys.characters(userId),
    queryFn: () =>
      requestJson<Character[]>("GET", `/api/dm/${userId}/characters`),
    enabled: Boolean(userId),
  });
}

export function useDmCharacterMutations(userId: string) {
  const queryClient = useQueryClient();
  const invalidateCharacters = () =>
    queryClient.invalidateQueries({ queryKey: dmQueryKeys.characters(userId) });

  const deleteCharacter = useMutation({
    mutationFn: async (id: string) => {
      await requestJson<{ success: boolean }>("DELETE", `/api/character/${id}`);
    },
    onSuccess: invalidateCharacters,
  });

  return { deleteCharacter, invalidateCharacters };
}
