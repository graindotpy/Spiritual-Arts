...
export {
  ...
  SAVE_ONLY_FOUNDRY_MECHANICS_VERSION,
  MAX_FOUNDRY_ACTIONS,
...
export const createCharacterSchema = insertCharacterSchema
  .omit({ portraitUrl: true, isDmOnly: true, dmOwnerId: true })
  .extend({ level: characterLevelSchema.default(1) })
  .strict();
export const createDmCharacterSchema = createCharacterSchema;
...
export type CreateDmCharacter = z.infer<typeof createDmCharacterSchema>;
...
export const cardGameStateDataSchema = z.record(z.string(), z.unknown());

export type CardGameStateData = z.infer<typeof cardGameStateDataSchema>;

export const cardGameStates = pgTable("card_game_states", {
  id: varchar("id").primaryKey(),
  state: jsonb("state").$type<CardGameStateData>().notNull(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export const insertCardGameStateSchema = createInsertSchema(cardGameStates, {
  id: z.string().trim().min(1).max(255),
  state: cardGameStateDataSchema,
})
  .omit({ createdAt: true, updatedAt: true })
  .strict();

export const cardGameStateRequestSchema = z
  .object({ state: cardGameStateDataSchema })
  .strict();
